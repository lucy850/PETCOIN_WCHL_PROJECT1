...
// Corrected session storage keys from 'happyCat' to 'happyRabbit' for consistency
let happy = parseInt(sessionStorage.getItem("happyRabbit")) || 100;
let hungry = parseInt(sessionStorage.getItem("hungryRabbit")) || 100;
let ekspresiTimeout = null;

// Renamed ekspresiCat to ekspresiRabbit for consistency. 
// Assuming the base images for expressions are `kelinci` (rabbit).
const ekspresiRabbit = {
    normal: "./png/kelinci.png",
    love: "./png/kelinci love.png",
    sad: "./png/kucing sedih.png" // This still uses a cat image, but is kept to preserve original behavior.
};

const itemData = {
    hair: [
        { name: "Long Rabbit Hair", price: 300, image: "./png/long kelinci.png" },
        { name: "Curly RabbitHair", price: 300, image: "./png/curly kelinci.png" },
        { name: "Short Rabbit Hair", price: 300, image: "./png/short kelinci.png" }
    ],
    // Corrected image names to be consistent with 'Cat' names
    accessories: [
        { name: "Cat Scarf", price: 300, image: "./png/syal kucing.png" },
        { name: "Cat Hat", price: 300, image: "./png/topi kucing.png" },
        { name: "Cat Necklace", price: 300, image: "./png/kalung kucing.png" }
    ],
    playTools: [
...
function updateBars() {
    ...
    // Corrected session storage keys from 'happyCat' to 'happyRabbit'
    sessionStorage.setItem("happyRabbit", happy);
    sessionStorage.setItem("hungryRabbit", hungry);
}
...
function showExpression(type, duration = 0) {
    ...
    // Corrected session storage key from 'itemTerpasangCat' to 'itemTerpasangRabbit'
    const terpasang = JSON.parse(sessionStorage.getItem("itemTerpasangRabbit")) || {};
    // Corrected to use ekspresiRabbit
    let gambarFinal = ekspresiRabbit[type];

    if (terpasang.hair) {
        const itemRambut = itemData.hair.find(i => i.name === terpasang.hair);
        if (itemRambut) {
            // Corrected expression image names to match `_senang.png` and `_nangis.png`.
            // You must ensure these image files exist.
            if (type === 'love') {
                gambarFinal = itemRambut.image.replace('.png', ' senang.png');
            } else if (type === 'sad') {
                gambarFinal = itemRambut.image.replace('.png', ' nangis.png');
            } else {
...
function feedPet() {
    ...
    // `coins` and `updateCoinDisplay` are not defined in the original snippet,
    // so this logic assumes they are defined elsewhere in your code.
...
function lepasItem(tipe) {
    ...
    sessionStorage.setItem("itemTerpasangRabbit", JSON.stringify(terpasang));
    showExpression("normal");
...
function handleItemAction(itemName, itemPrice, itemType, sudahDibeli) {
    ...
    if (sudahDibeli) {
        ...
        if (typeof coins !== 'undefined' && coins >= itemPrice) {
            ...
            // Corrected session storage key from 'koleksiItemCat' to 'koleksiItemRabbit'
            let koleksi = JSON.parse(sessionStorage.getItem("koleksiItemRabbit")) || {};
            if (!koleksi[itemType]) {
                koleksi[itemType] = [];
            }
            if (!koleksi[itemType].includes(itemName)) {
                koleksi[itemType].push(itemName);
                sessionStorage.setItem("koleksiItemRabbit", JSON.stringify(koleksi));
            }
            pakaiItem(itemName, itemType);
            bounceOnce();
...
function pakaiItem(itemName, itemType) {
    // Corrected session storage key from 'itemTerpasangCat' to 'itemTerpasangRabbit'
    let terpasang = JSON.parse(sessionStorage.getItem("itemTerpasangRabbit")) || {};
...
function populateShopModal(type) {
    ...
    const terpasang = JSON.parse(sessionStorage.getItem("itemTerpasangRabbit")) || {};
    // Corrected session storage key from 'koleksiItemCat' to 'koleksiItemRabbit'
    const koleksi = JSON.parse(sessionStorage.getItem("koleksiItemRabbit")) || {};
