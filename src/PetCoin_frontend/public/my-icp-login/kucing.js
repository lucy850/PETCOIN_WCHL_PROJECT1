...
let happy = parseInt(sessionStorage.getItem("happyCat")) || 100;
let hungry = parseInt(sessionStorage.getItem("hungryCat")) || 100;
let ekspresiTimeout = null;

const ekspresiCat = {
    normal: "./png/kucing123.png",
    love: "./png/kucing senang.png",
    sad: "./png/kucing nangis.png"
};

const itemData = {
    hair: [
        { name: "Long Cat Hair", price: 300, image: "./png/panjang kucing.png" },
        { name: "Curly Cat Hair", price: 300, image: "./png/curly kucing.png" },
        { name: "Short Cat Hair", price: 300, image: "./png/pendek kucing.png" }
    ],
    accessories: [
...
function updateBars() {
    ...
    sessionStorage.setItem("happyCat", happy);
    sessionStorage.setItem("hungryCat", hungry);
}
...
function showExpression(type, duration = 0) {
    ...
    const terpasang = JSON.parse(sessionStorage.getItem("itemTerpasangCat")) || {};
    let gambarFinal = ekspresiCat[type];
...
function lepasItem(tipe) {
    ...
    sessionStorage.setItem("itemTerpasangCat", JSON.stringify(terpasang));
    showExpression("normal");
...
function handleItemAction(itemName, itemPrice, itemType, sudahDibeli) {
    ...
    if (sudahDibeli) {
        ...
        if (typeof coins !== 'undefined' && coins >= itemPrice) {
            ...
            let koleksi = JSON.parse(sessionStorage.getItem("koleksiItemCat")) || {};
            if (!koleksi[itemType]) {
                koleksi[itemType] = [];
            }
            if (!koleksi[itemType].includes(itemName)) {
                koleksi[itemType].push(itemName);
                sessionStorage.setItem("koleksiItemCat", JSON.stringify(koleksi));
            }
            pakaiItem(itemName, itemType);
            bounceOnce();
...
function pakaiItem(itemName, itemType) {
    let terpasang = JSON.parse(sessionStorage.getItem("itemTerpasangCat")) || {};
...
function populateShopModal(type) {
    ...
    const terpasang = JSON.parse(sessionStorage.getItem("itemTerpasangCat")) || {};
    const koleksi = JSON.parse(sessionStorage.getItem("koleksiItemCat")) || {};
